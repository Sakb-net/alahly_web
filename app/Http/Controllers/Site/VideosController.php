...
class VideosController extends SiteController {
    ...
    public function index(Request $request) {
        if ($this->site_open == 1 || $this->site_open == "1") {
            ...
            //$data = Video::dataData($all_videos);
            return view('site.videos.index', compact('logo_image', 'data', 'type', 'page_title'))->with('i', ($request->input('page', 1) - 1) * 5);
        } else {
    ...
    public function single(Request $request, $link) {
        if ($this->site_open == 1 || $this->site_open == "1") {
            ...
            if (isset($data->id)) {
                ...
                $all_comment = $get_comment->get_commentdata($data, 'video');
