(function( $ ) {
	var calenderTpl = `
		...
		<div id="calTFooter">
			<h3 id="eventTitle">لا يوجد مباريات اليوم</h3>
			<a href="javascript:void(0);" id="calLink">ALL EVENTS</a>
		</div>
	`;
	var weekDaysFromSunday = '<div>أحد</div><div>اثنين</div><div>ثلاث</div><div>اربع</div><div>خميس</div><div>جمعة</div><div>سبت</div>';
	var weekDaysFromMonday = '<div>M</div><div>T</div><div>W</div><div>T</div><div>F</div><div>S</div><div>S</div>';
	var shortMonths = ["يناير", "فبراير", "مارس", "أبريل", "مايو", "يونيه", "يوليه","أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"];
    ...
    $.fn.miniEventCalendar = $.fn.MEC = function(options) {
        ...
        eventTitle.text("لا يوجد مباريات اليوم");
		...
		function populateCalendar(month, year, onInit) {
			...
			eventTitle.text("اضغط علي اليوم لمشاهدة المناسبة التي توجد به");
			eventsLink.text("المناسبة القادمة");
			eventsLink.attr("href", "#");
                        $('#calTFooter #calLink').css('display','none');
			curMonth = month;
			curYear = year;
		...
		function showEvent(event){
			if(event && event !== null && event !== undefined){
				...
				eventsLink.text("تفاصيل المباراة");
				eventsLink.attr("href", event.link);
			}else{
				eventTitle.text("لا يوجد مناسبة هذا اليوم");
				eventsLink.text("المناسبة القادمة");
				eventsLink.attr("href", settings.calendar_link);
			}
		}
