...
<tr>
    ...
    <td>{!! str_limit($comment->content,$limit = 50, $end = '...')  !!}</td>
    <!--<td>{{$comment->type}}</td>-->
    @if($comment_active == 1 )
    <td>
    ...
    <td>
        @if($comment_edit == 1)
        <a class="btn btn-primary fa fa-edit btn-user" data-toggle="tooltip" data-placement="top" data-title="{{trans('app.update')}} " href="{{ route('admin.productcomments.edit',$comment->id) }}"></a>
        <a class="btn btn-info fa fa-commenting btn-user" data-toggle="tooltip" data-placement="top" data-title="{{trans('app.view')}}  {{trans('app.replaies')}}  " href="{{ route('admin.productcomments.show',$comment->id) }}"></a>
        <a class="btn btn-success fa fa-plus btn-user" data-toggle="tooltip" data-placement="top" data-title="{{trans('app.add')}}  {{trans('app.replay')}} " href="{{ route('admin.productcomments.reply',$comment->id) }}"></a>
        @endif
        ...
        {!! Form::open(['method' => 'DELETE','route' => ['admin.productcomments.destroy', $comment->id],'style'=>'display:inline']) !!}
