...
<tr>
    ...
    <td>
<!--        @if($category_show == 1)
        <a class="btn btn-info fa fa-eye-slash"  data-toggle="tooltip" data-placement="top" data-title="عرض قسم فرعى" href="{{ route('admin.subcategories_product.show',$category->id) }}"></a>
        @endif-->
        @if($category_edit == 1)
        <a class="btn btn-primary fa fa-edit" data-toggle="tooltip" data-placement="top" data-title="تعديل قسم فرعى"  href="{{ route('admin.subcategories_product.edit',$category->id) }}"></a>
        @endif
        ...
        {!! Form::open(['method' => 'DELETE','route' => ['admin.subcategories_product.destroy', $category->id],'style'=>'display:inline']) !!}
